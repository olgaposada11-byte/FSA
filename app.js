...
async function syncConversationsToSupabase() {
    ...
    state.data.groups.forEach(group => {
        group.agent.conversations.forEach(
            (conversation, index) => {
                rows.push({
                    id: conversation.id,
                    group_id: group.id,
...
async function saveData() {
    try {
        ...
        const useCasesPromise =
            state.data.useCases.length
                ? supabaseClient.from("use_cases").upsert(
                    state.data.useCases.map(useCase => ({
                        id: useCase.id,
                        name: useCase.name,
        ...
        const groupsPromise =
            state.data.groups.length
                ? supabaseClient.from("groups").upsert(
        ...
        const assistantsPromise =
            state.data.groups.length
                ? supabaseClient.from("assistants").upsert(
                    state.data.groups.map(group => ({
                        group_id: group.id,
                        use_case_id:
        ...
        // así que solo se guardan una vez confirmados los grupos.
        const assistantsResult =
            groupsResult.error
                ? { error: null }
                : await assistantsPromise;
