...
function createInitialData() {
    ...
    return {
        ...
        groups: [
            ...
            createInitialGroup("EQUIPO01"),

            createInitialGroup("EQUIPO02"),

            createInitialGroup("EQUIPO03"),

            createInitialGroup("EQUIPO04")
...
function populateWizardFields(group) {
    ...
    setValue("agentInput", agent.inputs);
    setValue("agentOutput", agent.outputs);
...
function saveWizardData() {
    ...
    agent.inputs = getValue("agentInput");
    agent.outputs = getValue("agentOutput");
        ...
        <section class="use-case-detail">
            ...
            ${
                useCase.complexity
                    ? `
                        <p>
                            <strong>Complejidad sugerida:</strong>
                            ${escapeHtml(getComplexityLabel(useCase.complexity))}
                        </p>
                    `
                    : ""
            }
...
function bindSubmitActions() {
    ...
    const confirmationCheckbox =
        ...
        getElement("submissionConfirmationCheckbox"
...
function generateAssistantReportPdf(group) {
    ...
    doc.setFontSize(16);
    ensureSpace(24);
    doc.text(`Ficha del asistente — ${data.groupName}`, marginX, y);
    y += 28;

    writeHeading("Nombre del asistente");
    writeParagraph(data.assistantName);

    writeHeading("Descripción");
    writeParagraph(data.description);

    writeHeading("Caso de uso");
    writeParagraph(data.useCaseName);

    writeHeading("IA y modelo utilizado");
    writeParagraph(data.model);

    writeHeading("Nivel de complejidad");
    writeParagraph(data.complexity);

    writeHeading("Prompt completo");
    writeParagraph(data.prompt, { size: 9.5 });

    writeHeading("Conversaciones de ejemplo");

    if (!data.conversations.length) {
        writeParagraph("No se han registrado conversaciones de ejemplo.");
    } else {
        data.conversations.forEach((conversation, index) => {
            writeParagraph(`Ejemplo ${index + 1}:`, { bold: true });
            writeParagraph(`Usuario: ${conversation.userMessage || ""}`);
            writeParagraph(`Asistente: ${conversation.assistantMessage || ""}`);
        });
    }
...
function downloadGroupFactsheet(group) {
    ...
    const textReport = generateAssistantReportText(group);

    downloadFile(
        `${safeName}-informe.txt`,
        textReport,
...
function saveGroupFromModal() {
    ...
    const email =
        ...
        getValue("modalGroupEmail");

 

    const area =

        getValue("modalGroupArea");

 

    const members =

        getValue("modalGroupMembers")
    ...
    if (state.editingGroupId) {
        ...
        addActivity(
            ...
            `Se ha actualizado el grupo "${name}".`,
    ...
    const content = `
        <section class="agent-detail">
            ...
            <div class="detail-grid">
                ...
                <article class="detail-card">
                    <h3>Complejidad</h3>
                    <p>${escapeHtml(getComplexityLabel(agent.complexity))}</p>
                </article>
                ...
                <article class="detail-card">
                    <h3>Entradas</h3>
                    <p>${escapeHtml(agent.inputs || "Sin indicar")}</p>
                </article>

                <article class="detail-card">
                    <h3>Salidas</h3>
                    <p>${escapeHtml(agent.outputs || "Sin indicar")}</p>
                </article>
...
function bindExportActions() {
    ...
    const exportJsonButton =
        ...
        getElement("downloadEvaluatorJson") ||
